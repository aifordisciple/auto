...
@router.put("/{project_id}/files/{file_path:path}/content")
async def update_file_content(
    ...
    if not full_path.is_file():
        raise HTTPException(status_code=400, detail="只能编辑文件，不能编辑目录")

    # 检查 references 目录下的文件是否为只读
    if file_path.startswith("references/"):
        raise HTTPException(status_code=403, detail="references 目录下的文件为只读，禁止编辑")
