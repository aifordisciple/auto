...
- **结果解读指导**：
  ...
  3. GC Content: 双峰异常可能提示物种污染""",
    ...
    {
        "template_id": "singlecell_seurat_template",
        "name": "单细胞 RNA-seq 分析模板",
        "description": "基于 Seurat 的单细胞转录组分析流程，支持质控、聚类、细胞注释和差异分析",
        "template_type": TemplateType.PYTHON_ENV,
        "category": "single_cell",
        "category_name": "单细胞分析",
        "tags": ["scrna", "seurat", "single-cell", "clustering"],
        "parameters_schema": {
            "type": "object",
            "properties": {
                "input_format": {
                ...
                "input_paths": {
                    "type": "string",
                    "description": "输入数据路径，多个样本用逗号分隔"
                },
                "sample_names": {
                    "type": "string",
                    "description": "样本名称列表，逗号分隔"
                },
                "group_labels": {
                    "type": "string",
                    "description": "分组标签，逗号分隔"
                },
                "min_umi": {
                    "type": "integer",
                    "default": 1000,
                    "description": "最少 UMI 数阈值"
                },
                "min_genes": {
                    "type": "integer",
                    "default": 500,
                ...
                "integration_method": {
                    "type": "string",
                    "enum": ["cca", "rpca", "sct_harmony", "sct_cca"],
                    "default": "sct_harmony",
                    "description": "批次整合方法"
                },
                "output_dir": {
            ...
            "required": ["input_format", "input_paths", "sample_names", "output_dir"]
        },
        "expert_knowledge": """- **触发条件**：识别到"单细胞"、"scRNA"、"细胞聚类"等关键词
- **质控参数建议**：
  - 10x 数据推荐 min_umi=1000, min_genes=500
  - Smart-seq2 数据可适当降低阈值
  - 线粒体比例通常设 10-20%
- **整合方法选择**：
  - sct_harmony: 推荐，速度快、效果好
  - sct_cca: Seurat 标准方法
  - rpca: 大数据集推荐
- **常见问题**：
  1. 内存不足：减少 batchSize 或使用稀疏矩阵
  2. 整合效果差：检查批次效应来源""",
        "script_template": '''#!/usr/bin/env python3
"""
单细胞 RNA-seq 分析脚本模板 (Scanpy)

功能说明:
    基于 Scanpy 的单细胞分析流程，支持质控、降维、聚类和可视化

参数说明:
    --input_format: 输入数据格式 (10x/h5/mtx/rds)
    --input_paths: 输入数据路径，逗号分隔
    --sample_names: 样本名称，逗号分隔
    --group_labels: 分组标签，逗号分隔
    --min_umi: 最少UMI数阈值
    --min_genes: 最少基因数阈值
    --max_mt_percent: 线粒体比例上限
    --integration_method: 批次整合方法
    --output_dir: 输出目录
"""
...
def parse_args():
    ...
    parser.add_argument("--input_format", default="10x", help="Input format")
    parser.add_argument("--input_paths", required=True, help="Input paths, comma-separated")
    parser.add_argument("--sample_names", required=True, help="Sample names, comma-separated")
    parser.add_argument("--group_labels", required=True, help="Group labels, comma-separated")
    parser.add_argument("--min_umi", type=int, default=1000, help="Min UMI threshold")
    parser.add_argument("--min_genes", type=int, default=500, help="Min genes threshold")
    parser.add_argument("--max_mt_percent", type=float, default=15, help="Max MT percent")
    parser.add_argument("--integration_method", default="sct_harmony", help="Integration method")
...
def main():
    ...
    sample_names = [n.strip() for n in args.sample_names.split(",")]
    group_labels = [l.strip() for l in args.group_labels.split(",")]

    print(f"Loading {len(input_paths)} samples...")
...
if __name__ == "__main__":
    main()
''',
        "source_skill_id": "singlecell_seurat_pipeline_01",
        "is_official": True
    },
    {
        "template_id": "deseq2_deg_template",
        "name": "DESeq2 差异基因分析模板",
        "description": "RNA-seq 差异表达基因分析，支持多种实验设计，自动生成火山图和热图",
        "template_type": TemplateType.R_ENV,
        "category": "differential_analysis",
        "category_name": "差异分析",
        "subcategory": "degs",
        "subcategory_name": "差异基因",
        "tags": ["rnaseq", "deseq2", "deg", "differential-expression"],
        "parameters_schema": {
            "type": "object",
            "properties": {
                "count_matrix": {
                    "type": "string",
                    "format": "filepath",
                    "description": "基因计数矩阵文件 (TSV格式，行为基因，列为样本)"
                },
                "sample_info": {
                    "type": "string",
                    "format": "filepath",
                    "description": "样本信息文件 (TSV格式，包含 sample_id 和 group 列)"
                },
                "contrast": {
                    "type": "string",
                    "description": "比较组合，格式: treatment_vs_control"
                },
                "padj_cutoff": {
                    "type": "number",
                    "default": 0.05,
                    "description": "校正后 P 值阈值"
                },
                "log2fc_cutoff": {
                    "type": "number",
                    "default": 1.0,
                    "description": "log2 Fold Change 阈值"
                },
                "output_dir": {
                    "type": "string",
                    "format": "directorypath",
            ...
            "required": ["count_matrix", "sample_info", "contrast", "output_dir"]
        },
        "expert_knowledge": """- **数据格式要求**：
  - 计数矩阵：原始 counts，不要标准化
  - 样本信息：包含 sample_id 和 group 列
- **结果解读**：
  - padj < 0.05 且 |log2FC| > 1 视为显著差异
  - MA 图可评估整体表达模式
  - 火山图展示差异分布
- **常见问题**：
  1. 样本数过少：每组至少 3 个生物学重复
  2. 离群样本：检查 PCA 图，考虑剔除
  3. 低表达基因：建议预过滤""",
...
# 参数解析
option_list <- list(
    make_option(c("-c", "--count_matrix"), type="character", help="Count matrix file"),
    make_option(c("-s", "--sample_info"), type="character", help="Sample info file"),
    make_option(c("-n", "--contrast"), type="character", help="Contrast (e.g., treat_vs_ctrl)"),
    make_option(c("-p", "--padj_cutoff"), type="double", default=0.05, help="Adjusted p-value cutoff"),
    make_option(c("-l", "--log2fc_cutoff"), type="double", default=1.0, help="Log2FC cutoff"),
