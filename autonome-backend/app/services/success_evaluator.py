...
class SuccessEvaluator:
    ...
    def evaluate_code_complexity(self, code_blocks: List[Dict]) -> float:
        """
        评估代码复杂度，判断是否值得转化为技能

        Args:
            code_blocks: 代码块列表，每个元素包含 {"language": "python/r", "code": "..."}
        ...
        has_argparse = bool(re.search(r'argparse|optparse|commandArgs|sys\.argv', combined_code))
        if has_argparse:
            score += 0.3

        # 3. 输出操作检测 (权重 0.2)
        has_output = bool(re.search(r'to_csv|write\.|write\.table|save|export|to_tsv', combined_code))
        if has_output:
            score += 0.2

        # 4. 函数/类定义检测 (权重 0.1)
        has_function = bool(re.search(r'def |function |class ', combined_code))
        if has_function:
            score += 0.1

        # 5. 注释密度检测 (权重 0.1)
        comment_lines = len(re.findall(r'^\s*#|^\s*//|"""|\'\'\'', combined_code, re.MULTILINE))
        if comment_lines >= 3:
            score += 0.1

        return min(score, 1.0)

    def detect_executor_type(self, code_blocks: List[Dict]) -> str:
        """
        根据代码块检测执行器类型
        """
        if not code_blocks:
        ...
        languages = [block.get("language", "python").lower() for block in code_blocks]
    ...
    def should_trigger_skill_draft(
        self,
        session_id: str,
        ...
        try:
            ...
            strategies = self._extract_strategies(messages)

            result["code_blocks"] = code_blocks
            result["strategies"] = strategies

            # 3. 评估代码复杂度
            code_complexity = self.evaluate_code_complexity(code_blocks)

            # 4. 评估会话成功度
            success_result = self.evaluate_session(session_id)
            ...
            if has_output_files:
                ...
                if trigger_source == "none":
                    trigger_source = "output_file"

            # 触发条件4: 会话成功且置信度高
            if success_result["is_successful"] and success_result["confidence"] > 0.8:
                trigger_reasons.append(f"成功完成分析(置信度{success_result['confidence']:.0%})")
                trigger_score = max(trigger_score, success_result["confidence"])
                if trigger_source == "none":
                    trigger_source = "success_signal"
            ...
            result["trigger_reason"] = " | ".join(trigger_reasons) if trigger_reasons else "未满足触发条件"

            log.info(
                f"🎯 [SuccessEvaluator] 技能草稿触发判断: "
                f"触发={should_trigger}, 来源={trigger_source}, 评分={trigger_score:.2f}, "
                f"原因={result['trigger_reason']}"
            )

        except Exception as e:
            log.error(f"❌ [SuccessEvaluator] 触发判断失败: {e}")
            result["trigger_reason"] = f"判断异常: {str(e)}"

        return result

    def _extract_code_blocks(self, messages: List[ChatMessage]) -> List[Dict]:
        """
        从消息中提取代码块
        ...
        for msg in messages:
            if msg.role != RoleEnum.assistant:
                continue
    ...
    def _extract_strategies(self, messages: List[ChatMessage]) -> List[Dict]:
        """
        从消息中提取策略卡片
