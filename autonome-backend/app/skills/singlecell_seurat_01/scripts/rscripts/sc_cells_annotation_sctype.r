...
# # opt$infile <- "/mnt/beegfs/basic/single-cell/RX-7710478_Systemic_Sclerosis/basic/Merge/result/singlecell/1_preprocessing/sc_preprocessing.rds"

# # opt$dims <- 50
# opt$rdsfile <- "/data1/project/HNSCC_NC/basic/result/singlecell.sct_harmony/2_cells_analysis/cells_analysis.rds"
# # opt$samplelist <- "C04,C06,C07,C08,C09,C15,C21,C22,C26,C30,C31,C38,C43,C46,C51,C57,C59,C60,C84,C86,LN22,LN38,LN46,LN59,LP15,LP16,LP17,LP33,N06,N08,N09,N15,N21,N22,N38,N43,N46"
# opt$listname <- "A-NL,A-NL,A-NL,A-NL,A-NL,A-NL,A-NL,A-NL,A-NL,B-LP,B-LP,B-LP,B-LP,C-CA,C-CA,C-CA,C-CA,C-CA,C-CA,C-CA,C-CA,C-CA,C-CA,C-CA,C-CA,C-CA,C-CA,C-CA,C-CA,C-CA,C-CA,C-CA,C-CA,D-LN,D-LN,D-LN,D-LN"
# # opt$sctypedb = "/users/chengc/data/HNSCC/NC/basic/result/singlecell/2_cells_analysis/ScTypeDB_HNSCC.xlsx"
# # opt$tissue = "HNSCC"
# opt$cellanno <- "/data1/project/HNSCC_NC/basic/result/singlecell/2_cells_analysis/sctype/sctype_scores.xls"
# opt$skiptsne = "true"

# nohup Rscript sc_cells_annotation_sctype.r -l A-NL,A-NL,A-NL,A-NL,A-NL,A-NL,A-NL,A-NL,A-NL,B-LP,B-LP,B-LP,B-LP,C-CA,C-CA,C-CA,C-CA,C-CA,C-CA,C-CA,C-CA,C-CA,C-CA,C-CA,C-CA,C-CA,C-CA,C-CA,C-CA,C-CA,C-CA,C-CA,C-CA,D-LN,D-LN,D-LN,D-LN --rdsfile /data1/project/HNSCC_NC/basic/result/singlecell.sct_harmony/2_cells_analysis/cells_analysis.rds -d 50 -t "HNSCC" --sctypedb /data1/project/HNSCC_NC/basic/result/singlecell/2_cells_analysis/ScTypeDB_HNSCC.xlsx --cellanno /data1/project/HNSCC_NC/basic/result/singlecell/2_cells_analysis/sctype/sctype_scores.xls --defaultassay SCT --skiptsne true &
...
if(opt$export!="false"){
  ...
  write.table(sc.combined@assays[[opt$defaultassay]]@counts, file=gz1, quote=FALSE, sep='\t', col.names = TRUE)
  gz2 <- gzfile("Gene_Exp_per_Cell.tsv.gz", "w")
  write.table(sc.combined@assays[[opt$defaultassay]]@data, file=gz2, quote=FALSE, sep='\t', col.names = TRUE)
}
...
plot_cluster <- function(){
  ## tsne
  if(opt$skiptsne=="false"){
  p1 <- DimPlot(sc.combined, reduction = "tsne", group.by = "group", pt.size = 0.1)
  p2 <- DimPlot(sc.combined, reduction = "tsne", label = TRUE, pt.size = 0.1)
  # p1 <- AugmentPlot(plot = p1)
  # p2 <- AugmentPlot(plot = p2)

  pdf(file=paste("tsne_cluster","pdf",sep="."),width = 12,height = 5)
  # p1 <- DimPlot(sc.combined, reduction = "tsne", pt.size = 0.1, label = TRUE) + ggtitle(label = "FIt-SNE")
  # p1 <- AugmentPlot(plot = p1)
  print((p1+p2) + theme_paper)
  dev.off()

  png(file = paste("tsne_cluster",".png",sep=""),width = 300,height = 125,units = "mm",res = 300,pointsize = 10)
  par(mar = c(5, 5, 2, 2), cex.axis = 2, cex.lab = 2)
  # p1 <- DimPlot(sc.combined, reduction = "tsne", pt.size = 0.1, label = TRUE) + ggtitle(label = "FIt-SNE")
  # p1 <- AugmentPlot(plot = p1)
  print((p1+p2) + theme_paper)
  ...
  ## umap
  p1 <- DimPlot(sc.combined, reduction = "umap", group.by = "group", pt.size = 0.1)
  p2 <- DimPlot(sc.combined, reduction = "umap", label = TRUE, pt.size = 0.1)
  # p1 <- AugmentPlot(plot = p1)
  # p2 <- AugmentPlot(plot = p2)

  pdf(file=paste("umap_cluster","pdf",sep="."),width = 12,height = 5)
  print((p1+p2) + theme_paper)
  ...
  png(file = paste("umap_cluster",".png",sep=""),width = 300,height = 125,units = "mm",res = 300,pointsize = 10)
  par(mar = c(5, 5, 2, 2), cex.axis = 2, cex.lab = 2)
  print(p1+p2)
  # p1 <- DimPlot(sc.combined, reduction = "umap", pt.size = 0.1, label = TRUE) + ggtitle(label = "UMAP")
  # p1 <- AugmentPlot(plot = p1)
...
if(opt$skiptsne=="false"){
  ...
  pdf(file=paste("tsne_cluster_bygroup","pdf",sep="."),width = 4 * groupnum, height = 4)
  print(p1)
  dev.off()

  png(file = paste("tsne_cluster_bygroup",".png",sep=""),width = 100 * groupnum, height = 100,units = "mm",res = 300,pointsize = 10)
  par(mar = c(5, 5, 2, 2), cex.axis = 2, cex.lab = 2)
  print(p1)
  dev.off()
}
...
if(opt$cellanno!="" & opt$cellanno!="none"){
    # new.cluster.ids <- c("Naive CD4 T", "Memory CD4 T", "CD14+ Mono", "B", "CD8 T", "FCGR3A+ Mono", 
    #     "NK", "DC", "Platelet","Naive CD4 T 2", "Memory CD4 T 2", "CD14+ Mono  2", "B 2", "CD8 T 2", "FCGR3A+ Mono 2", 
    #     "NK 2", "DC 2", "Platelet 2", "other 2")
    # names(new.cluster.ids) <- levels(sc.combined)
    # sc.combined <- RenameIdents(sc.combined, new.cluster.ids)

    # cell_anno_dir = paste("","cell_anno", sep="")
    # if(!file.exists(cell_anno_dir)){
    #   dir.create(cell_anno_dir)
    # }
    # setwd(cell_anno_dir)

    DefaultAssay(sc.combined) <- opt$defaultassay

    # sc.combined <- RenameIdents(sc.combined, `0` = "CD14 Mono", `1` = "CD4 Naive T", `2` = "CD4 Memory T", 
    #     `3` = "CD16 Mono", `4` = "B", `5` = "CD8 T", `6` = "DC", `8` = "T activated", `7` = "NK", `9` = "B Activated", 
    #     `10` = "Mk", `11` = "pDC", `12` = "Other", `13` = "Eryth")

    sctype_scores=read.table(opt$cellanno,header = T,com = '',sep = "\t",quote = NULL,check.names = F)
    ...
    # gs_list = gene_sets_prepare("/users/chengc/work/pipeline/besaltpipe/src/SingleCell/sctype/ScTypeDB_short.xlsx", "Immune system") # e.g. Immune system, Liver, Pancreas, Kidney, Eye, Brain
    # gs_list = gene_sets_prepare("/users/chengc/work/pipeline/besaltpipe/src/SingleCell/sctype/ScTypeDB_full.xlsx", "Immune system") # e.g. Immune system,Pancreas,Liver,Eye,Kidney,Brain,Lung,Embryo,Gastrointestinal tract,Muscle,Skin
    ...
    # scRNAseqData <- RunPCA(scRNAseqData, features = VariableFeatures(object = scRNAseqData))
    ...
    # es.max = sctype_score(scRNAseqData = scRNAseqData[["integrated"]]@scale.data, scaled = TRUE, gs = gs_list$gs_positive, gs2 = gs_list$gs_negative)
...
system("perl /Users/chengchao/biosource/besaltpipe/src/SingleCell/allsample/sc_stat_cluster.pl")
...
#####################################################################################
## cluster注释细胞类型Assigning cell type identity to clusters
...
# sc.combined <- readRDS(file = "./cells_analysis.rds")
# conserved_markers <- readRDS(file = "./conserved_markers.rds")
# combined_markers <- readRDS(file = "./combined_markers.rds")
