...
# 1) 构建一个细胞信息表，用于给所有细胞打上最终的分组(subclone或其他)
...
# 注：cluster_df_fin 里只包含恶性细胞 => (Cell, OverClust_ID, Subclone)
#     其他细胞在 left_join 后 Subclone 会是 NA

# 2) 让“非恶性/未定”的 Subclone 显示特殊值
df_subclone_anno$Final_Subclone <- df_subclone_anno$Subclone
df_subclone_anno$Final_Subclone[ df_subclone_anno$Cell_Call=="Reference" ] <- "Reference"
df_subclone_anno$Final_Subclone[ df_subclone_anno$Cell_Call=="NonMalignant" ] <- "NonMalig"
df_subclone_anno$Final_Subclone[ df_subclone_anno$Cell_Call=="Unresolved" ] <- "Unresolved"
# 这样子一来：
#   恶性细胞 => "C1","C2" 等
#   非恶性 => "Reference"/"NonMalig"/"Unresolved"

# 3) 将矩阵转置(行=细胞,列=基因)，和前面类似
mat_t_sub <- t(as.matrix(cna_val))  # now row=Cell, column=Gene

# 4) 自定义行顺序
#    先按照 Final_Subclone 排序,
#    具体顺序: c("Reference","NonMalig","Unresolved","C1","C2","C3",...)
#    先获取实际出现的 subclone
sub_list <- unique(df_subclone_anno$Final_Subclone)
# 比如 sub_list 里包含 "Reference","NonMalig","C1","C2" 等
# 设计一个因子水平
ordered_levels <- c("Reference","NonMalig","Unresolved",
                    sort(sub_list[ grepl("^C", sub_list) ]))
# 其中 sort(...) 让 "C1", "C2" 按数字顺序排列(若是C10,C2,需特殊处理)
...
subclone_colors <- c("Reference"="gray70",
                     "NonMalig"="green3",
                     "Unresolved"="orange")
# 对“C1”、“C2”... 可手动指定，也可自动生成
other_cs <- all_subclones[ !(all_subclones %in% c("Reference","NonMalig","Unresolved")) ]
# 如果 subclone 较多，可自动分配颜色:
mycolor_1 <- c("#0072B2","#CC79A7","#009E73","#F0E442","#009E73","#CC79A7","#5d8ac4","#f39659","#936355","#999999","#E69F00","#56B4E9","#009E73","#F0E442","#0072B2","#D55E00","#E69F00","#56B4E9","#D55E00","#CC79A7","#FCFBFD","#EFEDF5","#DADAEB","#BCBDDC","#9E9AC8","#807DBA","#6A51A3","#54278F","#3F007D")
...
# 也可以设置 row_split = df_subclone_anno$Subclone_factor
# 这样每个 subclone 会分成独立 block

# 7) color mapping for cna
min_val <- min(cna_val)
max_val <- max(cna_val)
col_fun <- colorRamp2(c(min_val,0,max_val), c("blue","white","red"))
...
# 8) 输出 PDF
pdf("subclone_CNA_heatmap.pdf", width=8, height=6)
draw(ht_sub)
dev.off()

# 9) 输出 PNG (300 dpi, 单位为英寸)
png("subclone_CNA_heatmap.png", width=8, height=6, units="in", res=300)
draw(ht_sub)
dev.off()

cat("=== subclone_CNA_heatmap PDF/PNG 输出完成. ===\n")
