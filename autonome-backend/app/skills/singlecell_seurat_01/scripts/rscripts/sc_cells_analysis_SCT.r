...
if(opt$rdsfile==""){
  ...
  # sc.combined[["SCT"]] = JoinLayers(sc.combined[["SCT"]])
...
if(opt$rdsfile!=""){
  ...
  Idents(object = sc.combined) <- "SCT_snn_res.0.4"
  print("resolution:0.4")
  length(unique(factor(Idents(sc.combined))))
  Idents(object = sc.combined) <- "SCT_snn_res.0.6"
  print("resolution:0.6")
  length(unique(factor(Idents(sc.combined))))
  Idents(object = sc.combined) <- "SCT_snn_res.0.8"
  print("resolution:0.8")
  length(unique(factor(Idents(sc.combined))))
  Idents(object = sc.combined) <- "SCT_snn_res.1"
  print("resolution:1")
  length(unique(factor(Idents(sc.combined))))
  Idents(object = sc.combined) <- "SCT_snn_res.1.2"
  print("resolution:1.2")
  length(unique(factor(Idents(sc.combined))))
  Idents(object = sc.combined) <- "SCT_snn_res.1.4"
  print("resolution:1.4")
  length(unique(factor(Idents(sc.combined))))
  Idents(object = sc.combined) <- "SCT_snn_res.1.6"
...
#   # head(sc.combined@meta.data)
#   # # Assign identity of clusters
#   Idents(object = sc.combined) <- "SCT_snn_res.0.3"
...
if(opt$export!="false"){
  ...
  write.table(sc.combined@assays[["SCT"]]@counts, file=gz1, quote=FALSE, sep='\t', col.names = TRUE)
  gz2 <- gzfile("Gene_Exp_per_Cell.tsv.gz", "w")
  write.table(sc.combined@assays[["SCT"]]@data, file=gz2, quote=FALSE, sep='\t', col.names = TRUE)
}
...
es.max = sctype_score(scRNAseqData = scRNAseqData[["SCT"]]$scale.data, scaled = TRUE, gs = gs_list$gs_positive, gs2 = gs_list$gs_negative)
