...
option_list <- list(
  ...
  # Cell cycle
  make_option(c("--cell_cycle_enable"),
    action = "store", type = "character", default = "true",
    help = "true|false 是否计算 cell cycle 评分（默认 false）"
  ),
  make_option(c("--cell_cycle_species"),
    action = "store", type = "character", default = "human",
    help = "human|mouse 使用 Seurat cc.genes.updated.2019；mouse 会自动做首字母大写转换"
  ),
  make_option(c("--cell_cycle_regress"),
    action = "store", type = "character", default = "false",
    help = "true|false 是否在 ScaleData/SCTransform 中回归掉 cell cycle（默认 false）"
  ),
  make_option(c("--cell_cycle_regress_mode"),
    action = "store", type = "character", default = "scores",
    help = "scores|difference 回归变量用 S.Score+G2M.Score 或 CC.Difference"
  )
...
add_cell_cycle_scores <- function(obj, sample_id = NA_character_) {
  if (tolower(opt$cell_cycle_enable) != "true") return(obj)

  # Seurat 内置基因集
  s_genes  <- Seurat::cc.genes.updated.2019$s.genes
  g2m_genes <- Seurat::cc.genes.updated.2019$g2m.genes

  # mouse：把人类大写基因名转成 Mouse 风格（首字母大写，其余小写）
  if (tolower(opt$cell_cycle_species) == "mouse") {
    to_mouse <- function(x) paste0(toupper(substr(x, 1, 1)), tolower(substr(x, 2, nchar(x))))
    s_genes  <- to_mouse(s_genes)
    g2m_genes <- to_mouse(g2m_genes)
  }

  # 只保留对象中存在的基因，避免 warning/NA
  genes_present <- rownames(obj)
  s_use  <- intersect(s_genes, genes_present)
  g2m_use <- intersect(g2m_genes, genes_present)

  if (length(s_use) < 10 || length(g2m_use) < 10) {
    logw("[CellCycle] %s S/G2M genes too few after intersect: S=%d G2M=%d. Skip scoring.",
         sample_id, length(s_use), length(g2m_use))
    return(obj)
  }

  # 用 RNA assay 的 data（NormalizeData 后会有）
...
get_vars_to_regress <- function(obj, base_vars) {
  v <- base_vars

  if (tolower(opt$cell_cycle_enable) == "true" && tolower(opt$cell_cycle_regress) == "true") {
    if (tolower(opt$cell_cycle_regress_mode) == "difference") {
      if ("CC.Difference" %in% colnames(obj@meta.data)) v <- c(v, "CC.Difference")
    } else {
      if ("S.Score" %in% colnames(obj@meta.data))   v <- c(v, "S.Score")
      if ("G2M.Score" %in% colnames(obj@meta.data)) v <- c(v, "G2M.Score")
    }
  }

  # 只保留 meta.data 里存在的列
  v <- unique(v)
  v[v %in% colnames(obj@meta.data)]
}

plot_cell_cycle_qc <- function(obj, sid) {
  if (tolower(opt$cell_cycle_enable) != "true") return(invisible(NULL))
  need <- c("S.Score", "G2M.Score", "Phase", "CC.Difference")
  has_any <- any(need %in% colnames(obj@meta.data))
  if (!has_any) return(invisible(NULL))

  feats <- intersect(c("S.Score", "G2M.Score", "CC.Difference"), colnames(obj@meta.data))
  if (length(feats) == 0) return(invisible(NULL))

  p_v <- VlnPlot(obj, features = feats, ncol = length(feats), pt.size = 0.1) +
    plot_annotation(title = paste0("Cell cycle scores: ", sid))
  save_plot(
    p_v,
    paste0("QC_cell_cycle_", sid, ".pdf"),
    paste0("QC_cell_cycle_", sid, ".png"),
    max(6, 2.5 * length(feats)), 4
  )

  if ("Phase" %in% colnames(obj@meta.data)) {
    p_um <- NULL
    if ("umap" %in% names(obj@reductions)) {
      p_um <- DimPlot(obj, reduction = "umap", group.by = "Phase", pt.size = 0.1)
    } else {
      # 没有 umap 就用 pca
      ...
      p_um <- DimPlot(obj, reduction = "pca", group.by = "Phase", pt.size = 0.1)
    }
    save_plot(
      p_um,
      paste0("DimPlot_phase_", sid, ".pdf"),
      paste0("DimPlot_phase_", sid, ".png"),
      5, 4
    )
  }
...
run_doubletfinder_one <- function(obj, sample_id) {
  ...
  if (DefaultAssay(obj) == "RNA") {
    vt <- get_vars_to_regress(obj, base_vars = c("nFeature_RNA", "percent_mt"))
    obj <- ScaleData(obj, vars.to.regress = vt, verbose = FALSE)
  }
...
cell_filter_one <- function(i, countfiles, samplenames, listnames, formats, datasets) {
  ...
  # cell cycle scoring（必须在 NormalizeData 之后）
  obj <- add_cell_cycle_scores(obj, sample_id = sid)
  ...
  if (opt$method %in% c("sct_cca", "sct_rpca", "sct_harmony") && fmt != "sctrds") {
    vt <- get_vars_to_regress(obj, base_vars = c("percent_mt"))
    obj <- SCTransform(obj, method = "glmGamPoi", vars.to.regress = vt, verbose = FALSE)
  }
...
integrate_v5 <- function(obj, method) {
  ...
  if (use_sct) {
    ...
    vt <- get_vars_to_regress(obj, base_vars = character(0))
    if (length(vt) > 0) {
      obj <- ScaleData(obj, vars.to.regress = vt, verbose = FALSE)
    } else {
