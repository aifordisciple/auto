...
if(opt$rdsfile!=""){
  ...
  ## ===== 1. 重新挑选子集内的高变基因 =====
  #   - selection.method: "vst"（默认）/ "mean.var.plot"/"dispersion"
  #   - nfeatures: 子集通常 1 ~ 4 k 基因足够，按情况调整
  ...
  ## 可查看前 20 个高变基因
  head(VariableFeatures(sc.combined), 20)

  ## ===== 2. 只对这些高变基因做 Scale & PCA =====
  # sc.combined <- ScaleData(
  #   sc.combined,
  #   features = VariableFeatures(sc.combined),  # 减少计算量
  #   verbose  = FALSE
  # )
...
sc.combined <- RunPCA(sc.combined, features = VariableFeatures(object = sc.combined), npcs = 100)
  ...
  sc.combined <- FindClusters(sc.combined, resolution = seq(0.1,1.5,by=0.1),seed=TRUE)
...
if(opt$rdsfile==""){
  ...
  if(opt$skiptsne=="false"){
    sc.combined <- RunTSNE(sc.combined, dims = 1:opt$dims, tsne.method = "FIt-SNE", nthreads = 12, max_iter = 2000)
  }
...
# scRNAseqData <- FindVariableFeatures(scRNAseqData,nfeatures = opt$nfeatures)
...
# adata <- srt_to_adata(sc.combined)
# adata$write_h5ad("sc.h5ad")
...
## 挑选基因展示基因在各cluster中的表达情况。

# pdf(file=paste("FeaturePlot_tsne_gene","pdf",sep="."),width = 8,height = 4)
# p3 <- FeaturePlot(sc.combined, features = c("S100a9", "Sftpc"), reduction = "tsne", pt.size = 0.1, combine = FALSE)
# p3 <- lapply(X = p3, FUN = function(x) AugmentPlot(x + NoLegend()))
# # p3 <- lapply(X = p3, FUN = function(x) AugmentPlot(x + DarkTheme() + NoLegend()))
# wrap_plots(p3)
# dev.off()

# png(file = paste("FeaturePlot_tsne_gene",".png",sep=""),width = 200,height = 100,units = "mm",res = 300,pointsize = 2)
# par(mar = c(5, 5, 2, 2), cex.axis = 2, cex.lab = 2)
# p3 <- FeaturePlot(sc.combined, features = c("S100a9", "Sftpc"), reduction = "tsne", pt.size = 0.1, combine = FALSE)
# p3 <- lapply(X = p3, FUN = function(x) AugmentPlot(x + NoLegend()))
# # p3 <- lapply(X = p3, FUN = function(x) AugmentPlot(x + DarkTheme() + NoLegend()))
...
run_parallel_findallmarkers <- function(seurat_obj, ncores = 8, logfc = 0.25) {
...
# #### 2. FindConservedMarkers 找到各组中都保守的markers

# # https://zhuanlan.zhihu.com/p/134345603
# # 创建用来获取给定类群保留标记物的函数
# # get_conserved <- function(cluster){
# #   FindConservedMarkers(sc.combined,
# #                        ident.1 = cluster,
# #                        grouping.var = "group",
# #                        min.cells.group = 0,
# #                        only.pos = TRUE) %>%
...
# conserved_markers_dir = paste("","conserved_markers", sep="")
# if(!file.exists(conserved_markers_dir)){
#   dir.create(conserved_markers_dir)
# }
# setwd(conserved_markers_dir)
...
# # cbind_conserved_markers <- function(cluster){
# #   conserved_markers_list[[cluster+1]] %>%
# #   cbind(cluster = cluster, .)
# # }
...
# ## 大内存使用
# # conserved_markers_list <- future_lapply(maxIdents, get_conserved)
# # clulen=length(maxIdents)-1
# # conserved_markers <- map_dfr(c(0:clulen), cbind_conserved_markers)
...
# ## 小内存使用
# clulen=length(maxIdents)-1
# conserved_markers <- map_dfr(c(0:clulen), get_conserved)

# ##########################################################################################


# colname_len = length(colnames(conserved_markers))
# sample_num = (colname_len-4)/5
# avgfc <-c()
# for (i in 1:nrow(conserved_markers)) {
#     fcsum <<- 0
#     for (j in c(1:sample_num)) {
#         n = j*5 - 3 + 2
#         fcsum <<- fcsum+conserved_markers[i,n]
#     }
# 	fc <- fcsum/sample_num
# 	avgfc <- c(avgfc,fc)
# }

# conserved_markers <- cbind(conserved_markers,avgfc)
# # conserved_markers = conserved_markers[,1:19]
# conserved_markers <- conserved_markers %>%
#         dplyr::arrange(cluster, -avgfc)

# cat(c(''),file=paste('conserved_markers_export','.xls',sep=''))
# write.table(conserved_markers,file=paste('conserved_markers_export','.xls',sep=''),
#             append=T,quote=F,sep='\t',row.names = FALSE,col.names = TRUE)
...
# top3 <- conserved_markers %>% 
#   group_by(cluster) %>% 
#   top_n(n = 3, 
#         wt = avgfc)
# cat(c(''),file=paste('top3_conserved_markers_export','.xls',sep=''))
# write.table(top3,file=paste('top3_conserved_markers_export','.xls',sep=''),
#             append=T,quote=F,sep='\t',row.names = FALSE,col.names = TRUE)
...
#   top_n(n = 10, 
#         wt = avgfc)
# cat(c(''),file=paste('top10_conserved_markers_export','.xls',sep=''))
# write.table(top10,file=paste('top10_conserved_markers_export','.xls',sep=''),
#             append=T,quote=F,sep='\t',row.names = FALSE,col.names = TRUE)
...
#   top_n(n = 50, 
#         wt = avgfc)
# cat(c(''),file=paste('top50_conserved_markers_export','.xls',sep=''))
# write.table(top50,file=paste('top50_conserved_markers_export','.xls',sep=''),
#             append=T,quote=F,sep='\t',row.names = FALSE,col.names = TRUE)
...
#   top_n(n = 100, 
#         wt = avgfc)
# cat(c(''),file=paste('top100_conserved_markers_export','.xls',sep=''))
# write.table(top100,file=paste('top100_conserved_markers_export','.xls',sep=''),
#             append=T,quote=F,sep='\t',row.names = FALSE,col.names = TRUE)
...
# showgene = unique(diffgenes$gene)
# showgenenum = length(showgene)
# prefix = paste("top3_conserved_markers",sep="")
...
# #### 3. FindMarkers 找到一组和其他组比较的markers
# cluster_1vs2_markers_dir = paste("","cluster_1vs2_markers", sep="")
# if(!file.exists(cluster_1vs2_markers_dir)){
#   dir.create(cluster_1vs2_markers_dir)
# }
# setwd(cluster_1vs2_markers_dir)
# DefaultAssay(sc.combined) <- "RNA"
# # Determine differentiating markers for CD8+ T cells - clusters 6 versus 10
# cluster_1vs2_markers <- FindMarkers(sc.combined,
#                      ident.1 = 1,
#                      ident.2 = 2)
                     
# # find all markers of cluster 1
# # cluster1.markers <- FindMarkers(sc.combined, ident.1 = 1)
# # head(cluster1.markers, n = 5)

# # find all markers distinguishing cluster 5 from clusters 0 and 3
# # cluster5.markers <- FindMarkers(sc.combined, ident.1 = 5, ident.2 = c(0, 3))
# # head(cluster5.markers, n = 5)

# # Reorder columns and sort by log2 fold change        
# cluster_1vs2_markers <- cluster_1vs2_markers[, c(2:4,1,5)]

# # Arrange rows
# cluster_1vs2_markers <- cluster_1vs2_markers %>%
#         dplyr::arrange(p_val)

# # Save markers

# cat(c('Gene\t'),file=paste('cluster_1vs2_markers','.xls',sep=''))
# write.table(cluster_1vs2_markers,file=paste('cluster_1vs2_markers','.xls',sep=''),
...
# deg_up <- subset(cluster_1vs2_markers,avg_log2FC>=1)
# deg_up <- subset(deg_up,pct.1-pct.2>=0.25|pct.1-pct.2<=-0.25)
# deg_up <- subset(deg_up,p_val_adj<=0.05)

# cat(c('Gene\t'),file=paste('deg_up_cluster_1vs2_markers_export','.xls',sep=''))
# write.table(deg_up,file=paste('deg_up_cluster_1vs2_markers_export','.xls',sep=''),
#             append=T,quote=F,sep='\t',row.names = TRUE,col.names = TRUE)

# deg_up <- subset(cluster_1vs2_markers,avg_log2FC>=0)
# top3_up <- deg_up %>% 
#   top_n(n = 3, 
#         wt = avg_log2FC)
# cat(c('Gene\t'),file=paste('top3_up_cluster_1vs2_markers_export','.xls',sep=''))
# write.table(top3_up,file=paste('top3_up_cluster_1vs2_markers_export','.xls',sep=''),
#             append=T,quote=F,sep='\t',row.names = TRUE,col.names = TRUE)

# top10_up <- deg_up %>% 
#   top_n(n = 10, 
#         wt = avg_log2FC)
# cat(c('Gene\t'),file=paste('top10_up_cluster_1vs2_markers_export','.xls',sep=''))
# write.table(top10_up,file=paste('top10_up_cluster_1vs2_markers_export','.xls',sep=''),
#             append=T,quote=F,sep='\t',row.names = TRUE,col.names = TRUE)

# top50_up <- deg_up %>% 
#   top_n(n = 50, 
#         wt = avg_log2FC)
# cat(c('Gene\t'),file=paste('top50_up_cluster_1vs2_markers_export','.xls',sep=''))
# write.table(top50_up,file=paste('top50_up_cluster_1vs2_markers_export','.xls',sep=''),
#             append=T,quote=F,sep='\t',row.names = TRUE,col.names = TRUE)

# top100_up <- deg_up %>% 
#   top_n(n = 100, 
#         wt = avg_log2FC)
# cat(c('Gene\t'),file=paste('top100_up_cluster_1vs2_markers_export','.xls',sep=''))
# write.table(top100_up,file=paste('top100_up_cluster_1vs2_markers_export','.xls',sep=''),
#             append=T,quote=F,sep='\t',row.names = TRUE,col.names = TRUE)


# cluster_1vs2_markers <- cluster_1vs2_markers %>%
#         dplyr::arrange(avg_log2FC,p_val)
# deg_down <- subset(cluster_1vs2_markers,avg_log2FC<=-1)
# deg_down <- subset(deg_down,pct.1-pct.2>=0.25|pct.1-pct.2<=-0.25)
# deg_down <- subset(deg_down,p_val_adj<=0.05)

# cat(c('Gene\t'),file=paste('deg_down_cluster_1vs2_markers_export','.xls',sep=''))
# write.table(deg_down,file=paste('deg_down_cluster_1vs2_markers_export','.xls',sep=''),
#             append=T,quote=F,sep='\t',row.names = TRUE,col.names = TRUE)


# deg_down <- subset(cluster_1vs2_markers,avg_log2FC<0)
# top3_down <- deg_down %>% 
#   top_n(n = 3, 
#         wt = -avg_log2FC)
# cat(c('Gene\t'),file=paste('top3_down_cluster_1vs2_markers_export','.xls',sep=''))
# write.table(top3_down,file=paste('top3_down_cluster_1vs2_markers_export','.xls',sep=''),
#             append=T,quote=F,sep='\t',row.names = TRUE,col.names = TRUE)

# top10_down <- deg_down %>% 
#   top_n(n = 10, 
#         wt = -avg_log2FC)
# cat(c('Gene\t'),file=paste('top10_down_cluster_1vs2_markers_export','.xls',sep=''))
# write.table(top10_down,file=paste('top10_down_cluster_1vs2_markers_export','.xls',sep=''),
#             append=T,quote=F,sep='\t',row.names = TRUE,col.names = TRUE)

# top50_down <- deg_down %>% 
#   top_n(n = 50, 
#         wt = -avg_log2FC)
# cat(c('Gene\t'),file=paste('top50_down_cluster_1vs2_markers_export','.xls',sep=''))
# write.table(top50_down,file=paste('top50_down_cluster_1vs2_markers_export','.xls',sep=''),
#             append=T,quote=F,sep='\t',row.names = TRUE,col.names = TRUE)

# top100_down <- deg_down %>% 
#   top_n(n = 100, 
#         wt = -avg_log2FC)
# cat(c('Gene\t'),file=paste('top100_down_cluster_1vs2_markers_export','.xls',sep=''))
# write.table(top100_down,file=paste('top100_down_cluster_1vs2_markers_export','.xls',sep=''),
#             append=T,quote=F,sep='\t',row.names = TRUE,col.names = TRUE)
...
# # up 
# diffgenes  = top3_up
# showgene = unique(rownames(diffgenes))
# showgenenum = length(showgene)
# prefix = paste("top3_up_","cluster_1vs2_markers",sep="")
...
# showgene = unique(rownames(diffgenes))
# showgenenum = length(showgene)
# prefix = paste("top3_down_","cluster_1vs2_markers",sep="")
