...
option_list <- list(
    ...
    make_option(
        ...
        help = "细胞注释，cell replicate group"
    )
)
...
# min_val <- min(mat_t)
# max_val <- max(mat_t)
# col_fun <- colorRamp2(c(min_val,0,max_val), c("darkblue","white","darkred"))
...
#############################################################################
#   7. 示例热图, 按照恶性/非恶性分类对列做分组 + 按染色体分块 + 画竖线
#############################################################################
library(ComplexHeatmap)
library(circlize)
library(grid)
...
#------------------------------
# 0) 基因 -> 染色体映射，并按 chr + 起始位点排序
#------------------------------
gene2chr_file <- "/opt/data1/public/genome/human/annotation/gencode_v38/human_gene_pos.txt"
if (!file.exists(gene2chr_file)) {
  stop("gene2chr_file not found: ", gene2chr_file)
}
gene_chr_map  <- read.table(gene2chr_file, header = FALSE, sep = "\t",
                            stringsAsFactors = FALSE)
colnames(gene_chr_map) <- c("Gene","Chr","Start","End")

# 只保留在 cna_val 里的基因，按当前行名顺序匹配
gene_chr_sub <- gene_chr_map[match(rownames(cna_val), gene_chr_map$Gene), ]

# 染色体名统一成 chrX 形式
valid_idx <- which(!is.na(gene_chr_sub$Chr))
if (length(valid_idx) > 0 && any(!grepl("^chr", gene_chr_sub$Chr[valid_idx]))) {
  gene_chr_sub$Chr[valid_idx] <- paste0("chr", gene_chr_sub$Chr[valid_idx])
}

chr_levels <- c(paste0("chr", 1:22), "chrX", "chrY", "chrM")
gene_chr_sub$Chr <- factor(gene_chr_sub$Chr, levels = chr_levels)

# 按 chr + Start 排序（没有注释的基因排最后）
ord <- order(gene_chr_sub$Chr, gene_chr_sub$Start, na.last = TRUE)
cna_val      <- cna_val[ord, , drop = FALSE]
gene_chr_sub <- gene_chr_sub[ord, , drop = FALSE]

# 每个基因对应的染色体因子，后面做 column_split
chr_factor <- gene_chr_sub$Chr
chr_factor[is.na(chr_factor)] <- "Other"
chr_factor <- factor(chr_factor, levels = c(chr_levels, "Other"))

#------------------------------
# 1) 转置矩阵：行 = 细胞, 列 = 基因
#------------------------------
mat_t <- t(as.matrix(cna_val))
stopifnot(identical(colnames(mat_t), gene_chr_sub$Gene))

# 列分块：按染色体
chr_split <- chr_factor
names(chr_split) <- colnames(mat_t)

#------------------------------
# 2) 行分组（细胞分类）
#------------------------------
row_split <- factor(cell_call,
                    levels = c("Reference","NonMalignant","Malignant","Unresolved"))
...
#------------------------------
# 4) 底部染色体块注释：只在下面标一次 chr
#------------------------------
chr_used <- unique(chr_split)

ha_chr <- HeatmapAnnotation(
  Chr = anno_block(
    gp         = gpar(fill = NA, col = "black", lwd = 0.5),  # 画块边框
    labels     = as.character(chr_used),
...
#------------------------------
# 5) Heatmap 设置
#   - column_split = chr_split：按染色体切块
#   - column_title = NULL：去掉顶部 slice 名称（不再显示 chr1、chr2…）
#------------------------------
min_val <- min(mat_t)
max_val <- max(mat_t)
col_fun <- colorRamp2(c(min_val, 0, max_val), c("darkblue", "white", "darkred"))
...
ht_t <- Heatmap(
  ...
  show_row_names    = FALSE,
  show_column_names = FALSE,
  column_title      = NULL        # 关键：不在顶部绘制 chr 名称
)

#------------------------------
# 6) 输出 PNG 并手动画染色体边界竖线
#------------------------------
png("CNA_heatmap_transposed.png", width = 8, height = 5, units = "in", res = 300)
...
thr = as.numeric(signal_thr) * 1.25
# thr = 0.15
...
# 假设这个文件包含列：Cell、replicate、group、OtherInfo 等
annotation_file <- opt$cellanno
df_cell_anno <- read.table(annotation_file, header = TRUE, sep = "\t", stringsAsFactors = FALSE)

# 确认读入的数据，例如：
# head(df_cell_anno)
#   Cell   replicate group OtherInfo
# 1 cell1     S1     A      foo
# 2 cell2     S1     A      bar
# 3 cell3     S2     B      baz
# …
...
df_subclone_anno <- data.frame(
  ...
  # 额外合并用户提供的注释（可能包含 replicate、group、OtherInfo 等列）
  left_join(df_cell_anno, by = "Cell")

# 至此，df_subclone_anno 大概包含下列列：
#   Cell, Cell_Call, OverClust_ID, Subclone, replicate, group, OtherInfo
...
# 9.6.2 replicate 颜色映射（假设 df_subclone_anno$replicate 中有 "S1","S2",...）
sample_levels <- unique(df_subclone_anno$replicate)
...
ha_row2 <- rowAnnotation(
  ...
  replicate   = factor(df_subclone_anno$replicate, levels = sample_levels),
...
# ---------------------------
# （假设此前你已经得到了 df_subclone_anno 和 cna_val、cell_call、cluster_df_fin）
# ---------------------------

library(dplyr)
library(ggplot2)

# —— 1. 复用 Heatmap 时定义的 subclone_colors —— #
all_subclones <- unique(df_subclone_anno$Final_Subclone)
subclone_colors <- c(
  "Reference"  = "gray70",
  "NonMalig"   = "green3",
  "Unresolved" = "orange"
)
# 自动给 C1、C2… 等恶性子克隆分配颜色（与 Heatmap 时的配色保持一致）
mycolor_1 <- c("#0072B2","#CC79A7","#009E73","#F0E442","#5d8ac4",
               "#f39659","#936355","#999999","#E69F00","#56B4E9",
               "#D55E00","#FCFBFD","#EFEDF5","#DADAEB","#BCBDDC",
               "#9E9AC8","#807DBA","#6A51A3","#54278F","#3F007D")
other_cs <- setdiff(all_subclones, c("Reference", "NonMalig", "Unresolved"))
col_vec <- mycolor_1[seq_along(other_cs)]
names(col_vec) <- other_cs
subclone_colors <- c(subclone_colors, col_vec)

# —— 2. 计算每个 replicate 下各子克隆的 “细胞数” 和 “比例” —— #
df_prop <- df_subclone_anno %>%
  group_by(replicate, Final_Subclone) %>%
...
# —— 3. 把 replicate/Final_Subclone 都转成 factor，并指定顺序 —— #
# 假设你想让 replicate 的顺序和 Heatmap 中一致，就自己写一个向量
desired_sample_order <- unique(df_subclone_anno$replicate)
# 也可以直接 hard-code: c("P55","P76","P913", …)

df_prop <- df_prop %>%
  mutate(
    replicate = factor(replicate, levels = desired_sample_order),
    # 子克隆的水平顺序，严格按照 heatmap 中 subclone_colors 的名字
    Final_Subclone = factor(Final_Subclone, levels = names(subclone_colors))
  )

# —— 4. 用 ggplot2 画堆叠比例柱状图，并且用 subclone_colors 作为配色 —— #
p_subclone_prop <- ggplot(df_prop, aes(x = replicate, y = prop, fill = Final_Subclone)) +
  geom_bar(stat = "identity", width = 0.7) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
  scale_fill_manual(values = subclone_colors) +
  labs(x = "replicate", y = "Proportion", fill = "Subclone") +
...
# —— 5. 保存到文件 —— #
pdf("subclone_proportion_by_sample_sameColor.pdf", width = 8, height = 5)
print(p_subclone_prop)
dev.off()

png("subclone_proportion_by_sample_sameColor.png", width = 8, height = 5, units = "in", res = 300)
print(p_subclone_prop)
dev.off()
...
# —— 1. 从 df_prop 里过滤掉 Reference / NonMalig / Unresolved —— #
#    只保留名称以 "C" 开头（假设所有真正的子克隆名称都以 "C" 打头）
df_subclone_only <- df_prop %>%
  filter(grepl("^C", Final_Subclone))

# 如果你想更保险一点，也可以先把这三类显式地列出来，然后排除：
# df_subclone_only <- df_prop %>%
#   filter(!(Final_Subclone %in% c("Reference", "NonMalig", "Unresolved")))

# —— 2. 再次把 replicate 和 Final_Subclone 转成 factor，并指定顺序 —— #
#    一定要保证这里的 levels 与之前完全对应，否则 ggplot 会出现空白或顺序错乱
desired_sample_order <- unique(df_subclone_anno$replicate)
df_subclone_only <- df_subclone_only %>%
  mutate(
    replicate = factor(replicate, levels = desired_sample_order),
    Final_Subclone = factor(Final_Subclone, levels = names(subclone_colors))
  ) %>%
  # 既然不画那三类，就把它们的水平去掉
  droplevels()  # 删除在 factor levels 中已经排除掉的水平

# —— 3. 用 ggplot2 绘制堆叠比例柱状图，只包含真正子克隆 —— #
p_subclone_only <- ggplot(df_subclone_only, aes(x = replicate, y = prop, fill = Final_Subclone)) +
  geom_bar(stat = "identity", width = 0.7) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
  # 依然用同一个子克隆配色向量，但只取 names(subclone_colors) 里以 C 开头的部分
  scale_fill_manual(values = subclone_colors[grepl("^C", names(subclone_colors))]) +
  labs(x = "replicate", y = "Proportion", fill = "Subclone") +
  theme_bw(base_size = 12) +
  theme(
    axis.text.x       = element_text(angle = 45, hjust = 1),
...
# —— 4. 保存为文件 —— #
pdf("subclone_proportion_by_sample_onlyC.pdf", width = 8, height = 5)
print(p_subclone_only)
dev.off()

png("subclone_proportion_by_sample_onlyC.png", width = 8, height = 5, units = "in", res = 300)
print(p_subclone_only)
dev.off()
