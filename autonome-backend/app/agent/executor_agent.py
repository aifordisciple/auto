...
【当前上下文】
- 项目 ID: {project_id}
- 工作目录: /workspace/project_{project_id}/
- 任务输出目录: {env_info.get('TASK_OUT_DIR', f'/workspace/project_{project_id}/results/default')}
...
# 获取任务输出目录（已注入环境变量）
out_dir = os.environ.get('TASK_OUT_DIR', f'/workspace/project_{project_id}/results/default')
...
async def execute_single_task(
    ...
    enhanced_instruction: str = None,  # ✨ 新增：增强指令（包含调试错误信息或审稿意见）
    step_output_dir: str = None,  # ✨ 新增：语义化步骤输出目录
    step_number: int = None,  # ✨ 新增：步骤序号
) -> Dict[str, Any]:
    """
    执行单个 DAG 节点任务
    ...
    Args:
        ...
        enhanced_instruction: 增强指令（由 Debugger 或 Visual Reviewer 注入）
        step_output_dir: 语义化步骤输出目录（蓝图级联模式）
        step_number: 步骤序号（蓝图级联模式）
    ...
    # ✨ 使用增强指令（如果有），否则使用原始指令
    actual_instruction = enhanced_instruction or task.instruction

    # ✨ 构建环境变量注入
    # 如果有语义化步骤目录，优先使用；否则使用传统 task_id 目录
    if step_output_dir:
        task_out_dir = step_output_dir
        log.info(f"📁 [Executor] 使用语义化步骤目录: Step_{step_number:02d}")
    else:
        task_out_dir = f"/workspace/project_{project_id}/results/{task.task_id}"
