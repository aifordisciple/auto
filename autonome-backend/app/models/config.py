...
class SystemConfig(SQLModel, table=True):
    ...
    架构说明：
    - 思考模型配置：thinking_api_key, thinking_base_url, thinking_model 用于深度思考对话
    ...
    thinking_base_url: str = Field(default="https://api.openai.com/v1")  # 默认值与 config.settings.OPENAI_BASE_URL 保持一致
    thinking_model: str = Field(default="gpt-3.5-turbo")
    ...
    fast_api_key: Optional[str] = Field(default=None, description="极速模型API密钥，None表示回退到思考模型")
