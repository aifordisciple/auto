...
import { skillForgeApi, ExecutorType, CraftRequest, SkillTemplate, CraftResponse, SkillAsset } from '@/lib/api';
import { useWorkspaceStore } from '@/store/useWorkspaceStore';
import { Play, Hammer, Save, Send, Code, Terminal, FileJson, AlertTriangle, CheckCircle, FolderTree, GitBranch, X, Box, Sparkles, Upload, FileText, Folder, Loader2, FileCode, LayoutTemplate } from 'lucide-react';
...
// 主 Tab 类型
type MainTab = 'craft' | 'template' | 'myskills';
...
export function SkillForgeOverlay() {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (const file of files) {
      ...
      if (file.size > maxSize) {
        alert(`文件 ${file.name} 超过大小限制 (${file.type.startsWith('image/') ? '5MB' : '20MB'})`);
        continue;
      }
  ...
  const handleBundleUpload = async (file: File) => {
    ...
    setCraftLogs(`📦 正在解析压缩包: ${file.name}...\n`);
    ...
    try {
      // 自动切换到 Logical_Blueprint（适合多文件工作流）
      if (executorType !== 'Logical_Blueprint') {
        setExecutorType('Logical_Blueprint');
        setCraftLogs(prev => prev + "💡 检测到压缩包，自动切换到 Nextflow 工作流模式\n");
      }

      const result = await skillForgeApi.craftFromBundle({
        file: file,
        executorType: 'Logical_Blueprint',
        skillNameHint: skillNameHint || undefined,
        generateFullBundle: generateFullBundle
      });
      ...
      if (result.data.validation_warning) {
        ...
        setCraftLogs(prev => prev + "✅ 锻造成功！已生成 Nextflow 工作流。\n");
      }
      ...
      setCraftLogs(prev => prev + `❌ 压缩包解析失败: ${err.message}\n`);
      alert(`压缩包解析失败: ${err.message}`);
    } finally {
      ...
      if ((executorType === 'Python_env' || executorType === 'R_env') && result.data.script_code) {
        ...
        try {
          ...
          await skillForgeApi.testDraftSkillStream(
            ...
            // onLog - 实时日志回调
            (message) => {
              setCraftLogs(prev => prev + message + '\n');
            },
            // onStatus - 状态变化回调（可选）
            undefined,
            // onResult - 最终结果回调
          ...
          if (finalResult) {
            ...
            // 如果 AI 修复了代码，更新编辑器
            if (finalResult.final_code && finalResult.final_code !== result.data.script_code) {
              setScriptCode(finalResult.final_code);
              setCraftLogs(prev => prev + "\n🤖 Debugger 已自动修复代码。\n");
            }
          }
        } catch (testErr: any) {
...
`;
  ...
  // 处理模板实例化
  const handleTemplateInstantiate = (skillData: CraftResponse) => {
    // 设置锻造结果
    setCraftedSkill(skillData as any);
    setScriptCode(skillData.script_code || '');
    setSkillName(skillData.name);
    setSkillDescription(skillData.description);
    setExecutorType(skillData.executor_type as ExecutorType);
    setMainTab('craft'); // 切换回锻造 Tab
    setActiveEditorTab('code');
  };

  // 处理编辑已有技能
  const handleEditSkill = (skill: SkillAsset) => {
    // 设置锻造结果为已有技能
    setCraftedSkill(skill as any);
    setScriptCode(skill.script_code || '');
    setSkillName(skill.name);
    setSkillDescription(skill.description || '');
    setExecutorType(skill.executor_type as ExecutorType);
    setMainTab('craft'); // 切换回锻造 Tab
    setActiveEditorTab('code');
  };
            ...
            <div className="flex-1 flex flex-col h-screen overflow-hidden">
              ...
              <div className="flex-1 flex overflow-hidden">
                ...
                {mainTab === 'template' ? (
                  ...
                  /* 我的技能视图 */
                  <MySkillsPanel onEditSkill={handleEditSkill} />
                ) : (
                  <>
                {/* 左栏：输入与测试 */}
...
• 粘贴一段需要参数化的代码..."
                    ...
                    <div className="mt-2 shrink-0">
                      ...
                      {/* 压缩包文件预览 */}
                      {showBundlePreview && bundleFiles.length > 0 && (
                        <div className="mt-2 p-2 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
                          <div className="flex items-center justify-between mb-2">
                            <span className="text-xs font-medium text-blue-700 dark:text-blue-300 flex items-center gap-1">
                              <Folder size={12} />
                              压缩包解析结果 ({bundleFiles.length} 个文件)
                            </span>
                            <button
                          ...
                          <div className="max-h-32 overflow-y-auto space-y-0.5">
                            {bundleFiles.map((f, idx) => (
                              <div key={idx} className="flex items-center gap-2 px-2 py-0.5 bg-white dark:bg-neutral-800 rounded text-xs">
                                <FileCode size={10} className={f.type === 'script' ? 'text-green-500' : f.type === 'config' ? 'text-purple-500' : 'text-gray-400'} />
                                <span className="flex-1 truncate text-gray-700 dark:text-neutral-300">{f.path}</span>
                    ...
                    <button
                      onClick={handleCraft}
                      disabled={isCrafting || isBundleUploading}
