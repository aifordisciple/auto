 ...
 *
 * 注意：此组件支持两种模式：
 * 1. 带标题栏模式（默认）：包含可折叠标题栏
 * 2. 内嵌模式（showHeader=false）：只显示内容，用于嵌入父级折叠面板
 */
...
export function ParameterSchemaEditor({
  ...
  useEffect(() => {
    ...
    // 只有当 value 真正变化时才更新
    if (valueStr !== prevValueStr && Object.keys(value.properties || {}).length > 0) {
