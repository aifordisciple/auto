...
def peek_tabular_data(file_path: str, n_rows: int = 5) -> str:
    ...
    Args:
        file_path: 物理服务器上的绝对文件路径 (例如: /workspace/project_1/raw_data/counts.tsv)
...
def scan_workspace(directory_path: str) -> str:
    ...
    Args:
        directory_path: 需要扫描的绝对目录路径 (例如: /workspace/project_1/results/)
    """
...
- 读取路径：`/workspace/project_{project_id}/raw_data/`
- 写入路径：环境变量 `TASK_OUT_DIR` (默认 `/workspace/project_{project_id}/results/default_task`)。
- 所有的图表必须纯英文标签。
...
def main():
    ...
    df = pd.read_csv('/workspace/project_{project_id}/raw_data/ras.tsv', sep='\\t', index_col=0)
...
{{
  ...
  "tasks": [
    {{
      ...
      "depends_on": [],
      "expected_input": "/workspace/project_{project_id}/raw_data/matrix.mtx",
      "expected_output": "无",
    ...
    {{
      ...
      "depends_on": ["task_1"],
      "expected_input": "/workspace/project_{project_id}/raw_data/matrix.mtx",
      "expected_output": "/workspace/project_{project_id}/results/filtered_data.h5ad",
    ...
    {{
      ...
      "tool": "execute_python_code",
      "depends_on": ["task_2"],
      "expected_input": "/workspace/project_{project_id}/results/filtered_data.h5ad",
      "expected_output": "/workspace/project_{project_id}/results/pca_plot.tsv",
...
2. 物理目录：系统工作区挂载在 `/workspace/project_{project_id}/`，请确保代码中引用的路径都是合法的。
3. 代码规范：
