...
services:
  ...
  backend-api:
    volumes:
      - uploads_data:/workspace              # 具名卷：文件上传目录
...
| 数据持久化 | 具名卷 | `uploads_data:/workspace` |
...
- 上传文件路径：`/workspace/project_{project_id}/`
...
2. 图表：`plt.savefig(f'/workspace/project_{project_id}/result.png')`
